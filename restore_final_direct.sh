...
# Script final para restaurar todos los capítulos desde el 4 en adelante
...
restore_chapter() {
    ...
    # Buscar el archivo de backup correspondiente usando find
    local backup_file=$(find "$BACKUP_DIR" -name "Capítulo $chapter_num -*" | head -n 1)
    ...
    if [ "$chapter_num" -eq 1 ]; then
        ...
        local prev_file=$(find "$BACKUP_DIR" -name "Capítulo $prev_num -*" | head -n 1)
        if [ -n "$prev_file" ]; then
    ...
    local next_file=$(find "$BACKUP_DIR" -name "Capítulo $next_num -*" | head -n 1)
    if [ -n "$next_file" ]; then
